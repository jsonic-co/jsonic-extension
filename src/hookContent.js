;(() => {
  ...
  if (window.__JSONIC_EXTENSION_STATUS_PROXY__) {
    window.__JSONIC_EXTENSION_STATUS_PROXY__.status = "available"
  } else {
    window.__JSONIC_EXTENSION_STATUS_PROXY__ = defineSubscribableObject({
      status: "available",
    })
  }

  window.__JSONIC_EXTENSION_HOOK__ = {
    ...
    transformFormData: async (config) => {
      ...
      if (config.data instanceof File) {
        ...
        if (process.env.JSONIC_EXTENSION_TARGET === "CHROME") {
    ...
    cancelRequest: (config) => {
      window.postMessage(
        {
          type: "__JSONIC_EXTENSION_CANCEL__",
        },
        "*"
      )
    },
    ...
    sendRequest: (config) =>
      new Promise((resolve, reject) => {
        function handleMessage(ev) {
          ...
          if (ev.data.type === "__JSONIC_EXTENSION_RESPONSE__") {
            // Apply transformation from base64 to arraybuffer
            if (ev.data.isBinary) {
              const bytes = (ev.data.response.data.length / 4) * 3
              const ab = new ArrayBuffer(bytes)
              window.__JSONIC_EXTENSION_HOOK__.decodeB64ToArrayBuffer(
          ...
          } else if (ev.data.type === "__JSONIC_EXTENSION_ERROR__") {
        ...
        window.__JSONIC_EXTENSION_HOOK__
          ...
          .then((transformedConfig) => {
            window.postMessage(
              {
                type: "__JSONIC_EXTENSION_REQUEST__",
                config,
              },
              "*"
            )

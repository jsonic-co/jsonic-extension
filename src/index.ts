...
type JsonicExtensionRequestMeta = {
  timeData?: {
    startTime?: number
    endTime?: number
  }
...
): Promise<[Response, JsonicExtensionRequestMeta]> {
  const fetchHeaders = convertAxiosHeadersIntoFetchHeaders(axiosConfig.headers)

  const requestMeta: JsonicExtensionRequestMeta = { timeData: {} }
  ...
  try {
    ...
    if (objectURLsToRevoke.length > 0 && tabID) {
      chrome.tabs.sendMessage(
        tabID,
        {
          action: "__JSONIC_EXTENSION_REVOKE_OBJECT_URLS__",
          objectURLsToRevoke,
        },
        () => {}
      )
...
chrome.tabs.onUpdated.addListener((_id, _info, tab) => {
  if (tab.status !== "loading") {
    chrome.tabs.sendMessage(
      tab.id,
      {
        action: "__JSONIC_EXTENSION_PING__",
      },
      (_response: boolean) => {
